...
def write_to_csv(file_name):
    ...
    with open(f'{file_name}_regrets.csv', 'w', newline='') as file:
        row = csv.writer(file,  delimiter=';')
        for i, typ in enumerate(types):
            if typ in bidder_colors:
                data = np.array(
                    [[game_data_profile[i][d].regrets[t][-1] for t in range(0, T)] for d in range(len(game_data_profile[i]))])
                for d in data:
                    row.writerow([d[i] for i in range(len(d))])

    with open(f'{file_name}_bids.csv', 'w', newline='') as file:
        row = csv.writer(file,  delimiter=';')
        for i, typ in enumerate(types):
            if typ in bidder_colors:
                data = np.array(
                    [[game_data_profile[i][d].bids[t][-1] for t in range(0, T)] for d in range(len(game_data_profile[i]))])
                for d in data:
                    row.writerow([d[i] for i in range(len(d))])
    ...
    with open(f'{file_name}_payoffs.csv', 'w', newline='') as file:
        ...
        for i, typ in enumerate(types):
            if typ in bidder_colors:
                data = np.array(
                    [[game_data_profile[i][d].payoffs[t][-1] for t in range(0, T)] for d in range(len(game_data_profile[i]))])
                for d in data:
                    row.writerow([d[i] for i in range(len(d))])
